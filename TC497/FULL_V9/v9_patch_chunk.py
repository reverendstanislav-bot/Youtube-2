...
def load_manifest(path):
    return sorted(json.loads(Path(path).read_text(encoding="utf-8"))["patches"], key=lambda p: float(p["start"]))
...
def build_patch_ass(src_ass, patches, chunk_start, chunk_end, out_ass):
    ...
    for line in events.splitlines():
        ...
        if len(parts)<10:
            continue
        s0=ass_to_sec(parts[1]); e0=ass_to_sec(parts[2])
        for p in patches:
            ps=float(p["start"]); pe=float(p["end"])
            a=max(s0,ps,chunk_start); b=min(e0,pe,chunk_end)
            if b<=a:
                continue
            q=parts.copy()
    ...
    for p in patches:
        ...
        if not label:
            continue
        ps=float(p["start"]); pe=float(p["end"])
        dur=float(p.get("source_label_duration",2.2))
        a=max(ps,chunk_start); b=min(pe,ps+dur,chunk_end)
        if b<=a:
            continue
        rendered.append(
            "Dialogue: 0,"+sec_to_ass(a-chunk_start)+","+sec_to_ass(b-chunk_start)+
            ",Source,,0,0,0,,"+label
        )
...
def main():
    ...
    ass=Path(args.output).with_suffix(".patch.ass")
    ass_count=build_patch_ass(args.source_ass,patches,start,end,ass)

    cmd=["ffmpeg","-y","-loglevel","error","-ss",f"{start:.6f}","-i",args.input]
    image_patches=[]
    for p in active:
        path=(Path(args.asset_dir)/p["asset"]).resolve()
        if not path.is_file():
    ...
    for idx,(p,path) in enumerate(image_patches,start=1):
        ...
        crop=float(p.get("crop_fraction",0.12))
        keep=max(0.50,1.0-2.0*crop)
        img=f"img{idx}"; nxt=f"v{idx}"
        fc.append(
            f"[{idx}:v]crop=iw*{keep:.6f}:ih*{keep:.6f}:iw*{crop:.6f}:ih*{crop:.6f},"
            f"scale=960:540:force_original_aspect_ratio=increase,crop=960:540,"
            f"setpts=PTS-STARTPTS[{img}]"
        )
        fc.append(
    ...
    cmd += [
        ...
        "-map",prev,"-an","-t",f"{duration:.6f}","-r","30",
        "-c:v","libx264","-preset","ultrafast","-crf","19","-pix_fmt","yuv420p",
        "-g","60","-keyint_min","60","-sc_threshold","0",
        "-movflags","+faststart",args.output
    ]
    sh(cmd)
    sh(["ffprobe","-v","error","-show_entries","stream=codec_name,width,height,r_frame_rate:format=duration,size","-of","json",args.output])
