...
const keywords=new Set(['572','54','150','railroad','rails','highway','tc-497','longest','work','worked','scrapped']);
...
function Caption(){
  ...
  return <div style={{
    ...
    color:C.paper,textShadow:'0 2px 3px rgba(23,26,28,.95),0 0 8px rgba(23,26,28,.66)',
    opacity:fade(frame,p.s,p.e,3)
  }}>
    {p.words.map((w,i)=>{
      const clean=w.w.replace(/[.,!?;:]/g,'').toLowerCase();
      const rust=keywords.has(clean);
      return <React.Fragment key={i}><span style={{color:rust?C.rust:C.paper}}>{w.w}</span>{i<p.words.length-1?' ':''}</React.Fragment>;
    })}
  </div>;
}
