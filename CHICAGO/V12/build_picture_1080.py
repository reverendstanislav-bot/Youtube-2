...
def render_clean(v3,s,i,outdir):
    ...
    graph=f"[0:v]scale={WIDTH}:{HEIGHT}:force_original_aspect_ratio=increase:flags=lanczos,crop={WIDTH}:{HEIGHT},format=yuv444p"
...
def main():
    ...
    for i,s in enumerate(plan):
        ...
        if i%20==0:
            print(f"1080 picture {i+1}/{len(plan)}",flush=True)
    ...
    print(json.dumps({"shots":len(plan),"output":a.output_video,"size":[WIDTH,HEIGHT]},indent=2))
