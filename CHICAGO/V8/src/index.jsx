...
function Caption(){
  ...
  return <div style={{
    position:'absolute',
    left:endMode?54:70,
    right:endMode?390:70,
    top:undefined,
    bottom:endMode?38:26,
    textAlign:endMode?'left':'center',
    fontFamily:'Arial,Helvetica,sans-serif',
    fontWeight:760,
    fontSize:endMode?25:29,
    lineHeight:1.16,
    color:C.paper,
...
function EndScreen(){
  ...
  const local=frame-F(END_START);
  const brand=tween(frame,F(END_START)+4,10);
  const next=tween(frame,F(END_START)+18,10);
  const sub=tween(frame,F(END_START)+30,10);
  const scale=interpolate(
    local,[0,F(18)],[1.0,1.025],
    {...CLAMP,easing:Easing.inOut(Easing.cubic)}
  );

  return <AbsoluteFill style={{zIndex:30,overflow:'hidden',background:C.charcoal}}>
    <Img
      src={staticFile('end_bg.jpg')}
      style={{
    ...
    {/* very light cinematic grade only; no boxes/panels */}
    <div style={{
      position:'absolute',inset:0,
      background:'rgba(8,10,11,.14)'
    }}/>

    <div style={{
      position:'absolute',left:54,top:48,
      opacity:brand,
    ...
    {/* Real YouTube end-screen elements will be placed here later.
        We only label the clean zones — no fake rectangle or circle. */}
    <div style={{
      position:'absolute',right:58,top:148,
      width:285,
      opacity:next,
      transform:`translateX(${(1-next)*10}px)`,
      color:C.ivory,
      fontFamily:'Arial,Helvetica,sans-serif',
      textShadow:'0 2px 6px rgba(0,0,0,.72)'
    }}>
      <div style={{width:28,height:3,background:C.orange,marginBottom:10}}/>
      <div style={{fontSize:11,fontWeight:700,letterSpacing:2.5,color:C.orange}}>
        WATCH NEXT
      </div>
      <div style={{fontSize:20,fontWeight:800,letterSpacing:.7,marginTop:5}}>
        CONTINUE EXPLORING
      </div>
    </div>

    <div style={{
      position:'absolute',right:58,top:348,
      width:285,
      opacity:sub,
      transform:`translateX(${(1-sub)*10}px)`,
      color:C.ivory,
      fontFamily:'Arial,Helvetica,sans-serif',
      textShadow:'0 2px 6px rgba(0,0,0,.72)'
    }}>
      ...
      <div style={{fontSize:18,fontWeight:800,letterSpacing:.7,marginTop:5}}>
...
registerRoot(()=> <Composition id="ChicagoV8" component={Film} durationInFrames={32268} fps={25} width={960} height={540}/>);
