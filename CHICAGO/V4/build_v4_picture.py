...
def render_clean(v3,s,i,outdir):
    ...
    args += ["-filter_complex",graph,"-map","[v]","-frames:v",str(count),"-an",
             "-c:v","libx264","-preset","veryfast","-crf","19","-pix_fmt","yuv420p","-r","25","-g","50",
...
def main():
    ...
    for i,s in enumerate(plan):
        ...
        if i%20==0:
            print(f"clean picture {i+1}/{len(plan)}",flush=True)
    ...
    print(json.dumps({"shots":len(plan),"output":a.output_video},indent=2))
