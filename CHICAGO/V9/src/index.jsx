...
function EndScreen(){
  ...
  const subscribe=tween(frame,F(END_START)+18,10);
  ...
  return <AbsoluteFill style={{zIndex:30,overflow:'hidden',background:C.charcoal}}>
    ...
    {/* Canonical HIA 06 End Screen, first-episode variant:
        recommendation rectangle removed; subscribe/avatar circle preserved. */}
    ...
    <div style={{
      ...
      <div style={{marginTop:10,fontSize:12,fontWeight:600,letterSpacing:2.2,color:C.paper}}>
        THE INFRASTRUCTURE REMAINS
      </div>
    </div>

    {/* Canonical Subscribe circle center = approx (478,352) at 960x540.
        The real clickable Subscribe element is placed over this circle in YouTube Studio. */}
    <div style={{
      position:'absolute',
      left:399,top:424,width:160,
      opacity:subscribe,
      textAlign:'center',
      fontFamily:'Arial,Helvetica,sans-serif',
      color:C.ivory,
      ...
      <div style={{fontSize:11,fontWeight:700,letterSpacing:2.6,color:C.orange}}>
        SUBSCRIBE
      </div>
      <div style={{fontSize:13,fontWeight:700,letterSpacing:1.2,marginTop:5}}>
        HIDDEN INDUSTRIAL AMERICA
      </div>
    </div>
  </AbsoluteFill>;
...
registerRoot(()=> <Composition id="ChicagoV9" component={Film} durationInFrames={32268} fps={25} width={960} height={540}/>);
